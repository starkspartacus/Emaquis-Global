...
exports.listeProduitGlobal = async (req, res) => {
	if (req.session.user) {
		try {
			const { result: productsGlobal } =
				await produitQueries.getGlobalProduit();
	...
	if (user) {
		try {
			...
			const { result: product } = productId
				? await produitQueries.getGlobalProduitById(productId)
				: { result: null };
...
exports.postProduitGlobal = async (req, res) => {
	...
	try {
		...
		if (productId) {
			const { result: product } = await produitQueries.getGlobalProduitById(
				productId
			);
		...
		console.log(
			'🚀 ~ file: produitglobal.controler.js:110 ~ exports.postProduitGlobal= ~ error:',
			error
		);
		res.status(500).send({
			success: false,
...
exports.deleteProduitGlobal = async (req, res) => {
	try {
		...
		const product = await produitQueries.getGlobalProduitById(productId);

		if (!product.result) {
			throw Error('Product not found');
		}

		await produitQueries.deleteProduitGlobal(productId);
