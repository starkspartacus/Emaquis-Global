		...
		if (user) {
			...
			const updateProductStock = async () => {
				...
				for (let [index, produit_id] of body.produit.entries()) {
					...
					const quantite = Number(body.quantite[index]);

					if (produit) {
						produit.quantite += parseInt(quantite);

						if (produit.promo && produit.promo_quantity <= quantite) {
							let promo_quantity = parseInt(quantite / produit.promo_quantity);

							sum +=
								promo_quantity * produit.promo_price +
								(quantite % produit.promo_quantity) * produit.prix_vente;
						} else {
							sum += produit.prix_vente * quantite;
						}
